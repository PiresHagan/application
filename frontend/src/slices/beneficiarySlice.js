...
export const beneficiarySlice = createSlice({
  ...
  reducers: {
    ...
    associateBeneficiaryWithCoverage: (state, action) => {
      const { coverageId, beneficiaryId, relationship, allocation, type } = action.payload;
      ...
      if (existingIndex >= 0) {
        targetArray[existingIndex] = { beneficiaryId, relationship, allocation };
      } else {
        targetArray.push({ beneficiaryId, relationship, allocation });
      }
    },
